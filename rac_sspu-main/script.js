document.addEventListener('DOMContentLoaded', () => {
  ...
  if (navToggle && navList) {
    ...
    document.addEventListener('click', (e) => {
      if (navList.classList.contains('open') && !e.target.closest('.nav-list') && !e.target.closest('.nav-toggle')) {
        toggleMenu();
      }
    });
  ...
  // Enhanced Smooth Scrolling with offset and active section highlighting
...
// Create calendar dropdown menu
function createCalendarDropdown(btn) {
const dropdown = document.createElement('div');
dropdown.className = 'calendar-dropdown';

const options = [
{ name: 'Google Calendar', icon: 'fab fa-google' },
{ name: 'Outlook', icon: 'fab fa-windows' },
{ name: 'iCal', icon: 'fa fa-calendar' },
{ name: 'Download .ics', icon: 'fa fa-download' }
];

options.forEach(option => {
const optionEl = document.createElement('button');
optionEl.className = 'calendar-option';
optionEl.innerHTML = `<i class="${option.icon}"></i> ${option.name}`;
...
googleUrl.searchParams.append('dates', `${toICSDate(start)}/${toICSDate(end)}`);
...
case 'Outlook':
const outlookUrl = `https://outlook.live.com/calendar/0/action/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&location=${encodeURIComponent(location)}&body=${encodeURIComponent(description)}`;
...
'BEGIN:VCALENDAR',
'VERSION:2.0',
'PRODID:-//Rotaract SSPU//Events//EN',
'BEGIN:VEVENT',
`UID:${Date.now()}@rotaractsspu.org`,
...
// Close dropdown
dropdown.remove();
showToast(`Event added to ${option.name}`);
});
...
// Replace button content with icon and text
const originalHTML = btn.innerHTML;
btn.innerHTML = `<i class="far fa-calendar-plus"></i> ${originalHTML}`;
...
document.querySelectorAll('.calendar-dropdown').forEach(dropdown => dropdown.remove());

// Create and position new dropdown
const dropdown = createCalendarDropdown(btn);
